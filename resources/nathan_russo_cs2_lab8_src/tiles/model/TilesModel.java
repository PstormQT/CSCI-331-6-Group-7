...
public class TilesModel {
    ...
    public void move(Direction direction) {
        ...
        int total_points = moveTiles(direction);
        ...
        if (hasChanged) {
            ...
            if (status == GameStatus.ONGOING){
                message = String.format(STATE_MSGS.get(GameStatus.ONGOING), total_points);
            } else {
                message = STATE_MSGS.get(status);
            }
    ...
    private boolean saveScore() {
        ...
        if (originalBestScore < bestScore){
            originalBestScore = bestScore;
            ...
            try {
                PrintWriter out = new PrintWriter(new FileWriter(BEST_SCORE_FILE_NAME, false));
                out.append(Integer.toString(bestScore));
            } catch (IOException e) {
                return false;
            }
        }
        return true; // CHANGE THIS. It has been changed from false.
    }
